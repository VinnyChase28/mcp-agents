...
const logger = createLogger('math-mcp');
...
Help me understand which method might be best for different scenarios.`,
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
      ...
      logger.error('Argument parsing failed', { error });
    ...
    try {
      switch (name) {
        ...
        case "solve_quadratic":
          ...
          break;
        default:
          logger.error(`Tool implementation for '${name}' not found.`);
    ...
    logger.info(`Result for '${name}'`, { result: text });
